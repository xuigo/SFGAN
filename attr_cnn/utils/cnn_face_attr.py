""" train and test for a convolutional neural network for predicting face attrubute """
...
path_img = '/home/xushaohui/workspace_xsh/mydata/AttrData/FaceAttribute_128/gender/train_combine'
path_att = '/home/xushaohui/workspace_xsh/mydata/AttrData/FaceAttribute_128/gender/list_gender.txt'
path_model_save = './model'
attr='gender'

""" create path if not exist """
for path_used in [path_img, path_att, path_model_save]:
    if not os.path.exists(path_used):
        os.makedir(path_used)

name_model_save = os.path.join(path_model_save, 'model_{}.h5'.format(attr))
...
def get_data_info(path_img=path_img, path_att=path_att, yn_print_head_tail=False):
    """
...
def get_data_sample(img_idx=None, img_name=None, yn_interactive_plot=False):
    ...
    img = np.asarray(PIL.Image.open(os.path.join(path_img, img_name)))  # load image
...
def train_protocol():
    ...
    x_all, y_all = load_data_batch()
    history=model.fit(x=x_all, y=y_all, batch_size=256, epochs=100, verbose=1,
